...
function creaPostDeURLsEnITMT($conn, $datosReales){
        ...
        while( $registro = $result->fetch_row() ) {
                ...
                // Si la URL es un archivo local, hay que ponerle el prefijo de la ruta local
                if ( str_startsWith($url, '/archivos_secciones/') ) {
                    $url = RUTA_FILES . $url;
                }
...
function creaPaginaEstaticaPorID($conn, $idPagina){
    ...
    if( isset($conn) && isset($idPagina) ){
        ...
        while( $registro = $result->fetch_row() ) {
                ...
                //Creo el texto del POST
                if ($contenido){ // Si tiene contenido, lo separo de los enlaces con un BR
                    $texto = $contenido . "</br>";
                } else {
                    $texto = '';
                }

                // Creo los enlaces a las paginas hijas
                if ( $paginaHijas > 0 ){
                    $texto .= "<ul>";
